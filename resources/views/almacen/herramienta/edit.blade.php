...
@section ('contenido')
	<div class="row">
		<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
			<h3>Prestar herramienta:{{$herramienta->nombre}}</h3>
			@if (count($errors)>0)
      ...
      {!!Form::model($herramienta,['method'=>'PATCH','route'=>['herramienta.update',$herramienta->num_progre]])!!}
            {{Form::token()}}
                    	...
                    	<input type="text" name="num_progre" class="form-control" value="{{$herramienta->num_progre}}" placeholder="Num progre ...">
                    </div>

										<div class="form-group">
												<label for="id">Clave</label>
												<input type="text" name="id" class="form-control" value="{{$herramienta->id}}">
										</div>
										<div class="form-group">
										<label for="nombre">Nombre herramienta</label>
										<input type="text" name="nombre" class="form-control" value="{{$herramienta->nombre}}" placeholder="Nombre herramienta...">
									</div>
									<div class="form-group">
										<label for="categoria">Categoria</label>
										<select name="categoria" class="form-control">
											<option value="{{$herramienta->categoria}}">{{$herramienta->categoria}}</option>
										...
										<div class="form-group">
											...
											<input type="text" name="cantidad" class="form-control" value="{{$herramienta->cantidad}}" placeholder="Cantidad...">
										</div>

											<div class="form-group">
												<label for="unidad">Unidad</label>
												<select name="unidad" class="form-control">
													<option value="{{$herramienta->unidad}}">{{$herramienta->unidad}}</option>
													<option>1</option>
													<option>2</option>
													<option>3</option>
													<option>4</option>
